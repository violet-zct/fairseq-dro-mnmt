...
class TokenWeightedLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def __init__(self, task, sentence_avg, label_smoothing, obj, start_ft_steps, adaptive_T):
        super().__init__(task)
        ...
        self.obj = obj
        median = np.median(task.target_dictionary.count)
        counts = torch.FloatTensor(task.target_dictionary.count)

        if obj == 'exp':
            weights = torch.exp(-1 * adaptive_T * counts / median)
        else:
            weights = torch.pow(counts / median, 2) * torch.exp(-1 * adaptive_T * counts / median)
        weights = weights / weights.max() * (np.e - 1) + 1
    ...
    def add_args(parser):
        ...
        parser.add_argument('--obj', default="exp", choices=['exp', 'chi_square'])
        parser.add_argument('--adaptive-T', type=float)
        # fmt: on
