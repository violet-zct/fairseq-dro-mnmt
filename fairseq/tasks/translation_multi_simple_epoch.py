...
class TranslationMultiSimpleEpochTask(LegacyFairseqTask):
    ...
    def train_dynamic_step(self, sample, model, criterion):
        model.eval()
        with torch.no_grad():
            loss, sample_size, logging_output, sample_ids, hardness_metric = \
                criterion(model, sample, train_dynamic=True)
        return loss, sample_size, logging_output, sample_ids, hardness_metric
    ...
    def get_batch_iterator(
        ...
        if 'concat_train' in self.datasets and dataset == self.datasets['concat_train']:
            self.dataset_to_epoch_iter['concat_train'] = epoch_iter
        return epoch_iter
