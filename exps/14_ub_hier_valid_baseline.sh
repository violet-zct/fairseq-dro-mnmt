...
exp_name=14_valid_bl_outer_ub_hier_ema0.01_alpha0.5_beta0.5_wd1e-4_dro_ted8_${ename}
...
python train.py ${DATA}\
    --start-ft-steps 25000 --valid-baseline 1 \
    --task translation_multi_simple_epoch \
    --arch ${model} --valid-subset cap.valid \
    --encoder-langtok ${etok} --enable-lang-ids --log-path ${SAVE}/inner_log.txt \
    --criterion 'outer_upper_bound_hier_dro_label_smoothed_cross_entropy' --label-smoothing 0.1 \
    --dro-outer-alpha 0.5 --dro-inner-beta 0.5 --update-dro-freq 1000 --outer-group-level ${glevel} --ema 0.01 \
